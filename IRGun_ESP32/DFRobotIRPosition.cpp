...
bool DFRobotIRPosition::available()
{
    ...
    { // read only the data lenth fits.
        ...
        for (int i = 0; i < 4; i++)
        {
            positionX[i] = (uint16_t)(positionData.positionFrame.rawPosition[i].xLowByte) + ((uint16_t)(positionData.positionFrame.rawPosition[i].xyHighByte & 0x30U) << 4);

            positionY[i] = (uint16_t)(positionData.positionFrame.rawPosition[i].yLowByte) + ((uint16_t)(positionData.positionFrame.rawPosition[i].xyHighByte & 0xC0U) << 2);
        }
        return true;
    }
    else
